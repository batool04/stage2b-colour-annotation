...
if not st.session_state.started:
    st.title("🎨 Stage 2B — Concept-to-Palette Annotation")

    st.info(
        "Quick task: choose your assigned rater ID, then for each concept pick "
        "1–3 palette colours and press Save & Next."
    )
    ...
    if start:
        ...
        if resume_file is not None:
            ...
            # Start a fresh session unless the rater explicitly uploads
            # their own previously downloaded progress CSV.
            annotations = {}
    ...
    st.warning(
        "Progress is not automatically restored from the cloud. "
        "Download your CSV before leaving, then upload it later to resume."
    )
    st.stop()
...
st.markdown(
    f'<div class="topline"><b>{rater_id}</b> &nbsp;•&nbsp; '
    f'Concept {idx + 1}/50 &nbsp;•&nbsp; {completed_count}/50 saved '
    f'&nbsp;•&nbsp; {category}</div>',
    unsafe_allow_html=True,
)

st.markdown(
    f'<div class="concept-title">{concept.upper()}</div>',
    unsafe_allow_html=True,
)

st.markdown(
    f'<div class="guide-box"><b>Meaning:</b> {guide}</div>',
    unsafe_allow_html=True,
)

# Restore saved values when revisiting an annotated concept.
...
st.markdown(
    '<div class="section-label">1. Choose broad colour family/families</div>',
    unsafe_allow_html=True,
)

# Compact family navigation in two rows.
family_cols_1 = st.columns(5, gap="small")
family_cols_2 = st.columns(5, gap="small")
families = []
for i, fam in enumerate(FAMILIES):
    col = family_cols_1[i] if i < 5 else family_cols_2[i - 5]
    key = f"family_{cid}_{fam}"
    default = fam in st.session_state[family_key]
    with col:
        checked = st.checkbox(fam, value=default, key=key)
    if checked:
        families.append(fam)

st.session_state[family_key] = families
...
# Full palette is optional and hidden by default.
full_key = f"full_palette_{cid}"
if full_key not in st.session_state:
    st.session_state[full_key] = False

opt1, opt2 = st.columns([1.25, 5.75])
with opt1:
    if not st.session_state[full_key]:
        if st.button("Browse all 448", key=f"show_all_{cid}", use_container_width=True):
            st.session_state[full_key] = True
            st.rerun()
    else:
        if st.button("Hide full palette", key=f"hide_all_{cid}", use_container_width=True):
            st.session_state[full_key] = False
            st.rerun()

with opt2:
    st.markdown(
        '<div class="optional-note">Use the full palette only if the family view '
        'does not contain a suitable colour.</div>',
        unsafe_allow_html=True,
    )

view_all = bool(st.session_state[full_key])

if view_all:
    visible = palette
    visible_title = "Full palette"
elif families:
    visible = palette[palette["family"].isin(families)]
    visible_title = "Selected family palette"
else:
    visible = palette.iloc[0:0]
    visible_title = ""
if visible.empty and not view_all and not families:
    st.info("Choose one or more broad colour families above to see matching palette colours.")
else:
    st.markdown(
        f'<div class="section-label">2. Pick 1–3 palette colours '
        f'({len(visible)} shown)</div>',
        unsafe_allow_html=True,
    )

    # Compact 12-column palette grid: more colours fit on one screen.
    rows_list = list(visible.to_dict("records"))
    for start_i in range(0, len(rows_list), 14):
        cols = st.columns(14, gap="small")
        for j, sw in enumerate(rows_list[start_i:start_i + 14]):
            pid = sw["palette_id"]
            hexv = str(sw["HEX"])
            ...
            try:
                ...
                fg = "#111111" if lum > 155 else "#ffffff"
            except Exception:
                fg = "#111111"

            with cols[j]:
                st.markdown(
                    f'<div class="swatch-mini" style="background:{hexv};color:{fg};">'
                    f'{pid}</div>',
                    unsafe_allow_html=True
                )
                choice = st.checkbox(
...
if len(selected_ids) > 3:
    st.error(
        f"You selected {len(selected_ids)} colours. Keep only 1–3."
    )
elif selected_ids:
    st.success(
        "Selected: " +
    ...
    st.caption("Select 1–3 swatches. Selection order = primary → secondary → tertiary.")
...
st.markdown('<div class="sticky-help"></div>', unsafe_allow_html=True)

nav1, nav2, nav3, nav4 = st.columns([1, 1.4, 1.55, 1.15])
    ...
    if st.button("Save & Next →", type="primary", use_container_width=True):
        if save_current():
            if idx >= len(concepts) - 1:
                st.success("50/50 complete. Download the final CSV.")
            else:
                st.session_state.current_idx = idx + 1
                reset_concept_widget_state()
                st.rerun()
...
with nav4:
    if st.button("Change rater", use_container_width=True):
...
if len(st.session_state.annotations) == len(concepts):
    st.success("✅ Annotation complete: 50 / 50. Download the CSV and send it to the researcher.")
    st.download_button(
        "⬇ Download FINAL CSV",
...
with st.expander("Session / safety information"):
    ...
    st.write(
        "Download your progress CSV before leaving. To resume later, upload that "
        "same CSV from the start screen."
    )
