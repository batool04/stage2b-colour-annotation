...
st.markdown(
    ...
    /* Reduce default Streamlit vertical gaps */
    ...
    /* Make Streamlit buttons/checks tighter */
    .stButton > button {
        min-height: 2rem;
    ...
    /* Sidebar-style compact section text */
    .tiny-note {
        font-size: 0.78rem;
        color: #5f6368;
        line-height: 1.2;
...
if not st.session_state.started:
    ...
    st.warning(
        "Download your progress CSV before closing the browser. "
        "Upload that CSV later if you need to resume."
    )
    st.stop()
...
with top1:
    ...
    st.caption(f"{completed_count}/50 completed • Concept {idx + 1}/50")
with top2:
    st.markdown(f"**Rater:** `{rater_id}`")
with top3:
    st.caption(f"Stratum: {category}")

st.markdown(f'<div class="concept-title">{concept.upper()}</div>', unsafe_allow_html=True)

st.markdown(
    f'<div class="guide-box"><b>Concept guide:</b> {guide}</div>',
    unsafe_allow_html=True
)
...
ctrl1, ctrl2, ctrl3 = st.columns([4.4, 1.15, 1.35])
...
with ctrl1:
    families = st.multiselect(
        ...
        key=f"family_{cid}",
        placeholder="Choose family/families",
...
with ctrl2:
    ...
    view_all = st.checkbox(
        ...
        key=f"view_all_{cid}",
        help="Show the full frozen palette.",
    )
...
if visible.empty and not view_all and not families:
    ...
    for start_i in range(0, len(rows_list), 12):
        cols = st.columns(12, gap="small")
        for j, sw in enumerate(rows_list[start_i:start_i + 12]):
            pid = sw["palette_id"]
            hexv = str(sw["HEX"])
...
with nav4:
    if st.button("End / change rater", use_container_width=True):
